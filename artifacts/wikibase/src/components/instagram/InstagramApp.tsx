...
function PostCard({
  post, profiles, editor, onProfile, onToggleLike, onToggleSave, onOpen, onRegenerate,
}: {
  post: InstagramPost; profiles: InstagramProfile[]; editor: boolean; onProfile: (id: string) => void;
  onToggleLike: () => void; onToggleSave: () => void; onOpen: () => void; onRegenerate: () => void;
}) {
  ...
  return (
    <article className="ig-post">
      <header className="ig-post-header">
        ...
        <button className="ig-icon-button" aria-label="Plus d’options"><MoreHorizontal size={21} /></button>
      </header>
      ...
      <div className="ig-post-body">
        ...
        {post.comments.length > 0 && <button className="ig-comments-link" onClick={onOpen}>Afficher les {post.comments.length} commentaires</button>}
...
function PostDetail({ post, profiles, editor, onClose, onProfile, onAddComment, onDeleteComment }: { post: InstagramPost; profiles: InstagramProfile[]; editor: boolean; onClose: () => void; onProfile: (id: string) => void; onAddComment: (text: string) => void; onDeleteComment: (commentId: string) => void }) {
  ...
  return <Overlay title="Publication" onClose={onClose}><div className="ig-post-detail">
    ...
    <div className="ig-detail-comments">
      <div className="ig-detail-author"><Avatar profile={author} size={36} onClick={() => onProfile(author.id)} /><p><b>{author.username}</b><br /><MentionText text={post.caption} profiles={profiles} onProfile={onProfile} /></p></div>
      <div className="ig-comment-list">{post.comments.length === 0 ? <p className="ig-empty-small">Pas encore de commentaire.</p> : post.comments.map(item => {
        const commenter = profiles.find(profile => profile.id === item.authorId);
        return commenter ? <div className="ig-comment" key={item.id}><Avatar profile={commenter} size={28} onClick={() => onProfile(commenter.id)} /><p><b>{commenter.username}</b><br />{item.text}<small>{relativeTime(item.createdAt)}</small></p>{editor && <button className="ig-comment-delete" onClick={() => onDeleteComment(item.id)} aria-label="Supprimer ce commentaire"><Trash2 size={14} /></button>}</div> : null;
      })}</div>
      <form className="ig-comment-form" onSubmit={event => { event.preventDefault(); if (comment.trim()) { onAddComment(comment.trim()); setComment(''); } }}>
...
export function InstagramApp({ pages }: { pages: WikiPage[] }) {
  ...
  const addComment = (postId: string, text: string) => {
    if (!viewer) return;
    const comment: InstagramComment = { id: `ig-comment-${Date.now()}`, authorId: viewer.id, text, createdAt: Date.now(), likes: 0 };
    updateDatabase(current => ({ ...current, posts: current.posts.map(post => post.id === postId ? { ...post, comments: [...post.comments, comment] } : post) }));
  };
  const regenerate = async (post: InstagramPost) => {
    ...
    editPost(post.id, { comments: generated.map((comment, index) => ({ id: `ig-ai-${Date.now()}-${index}`, authorId: comment.authorId, text: comment.text, createdAt: Date.now(), likes: Math.floor(Math.random() * 48) })) });
  ...
  return <div className="instagram-app">
    ...
    {selectedPost && <PostDetail post={database.posts.find(post => post.id === selectedPost.id) ?? selectedPost} profiles={profiles} editor={editor} onClose={() => setSelectedPost(null)} onProfile={openProfile} onAddComment={text => addComment(selectedPost.id, text)} onDeleteComment={commentId => updateDatabase(current => ({ ...current, posts: current.posts.map(post => post.id === selectedPost.id ? { ...post, comments: post.comments.filter(comment => comment.id !== commentId) } : post) }))} />}
    {modal === 'post' && <CreatePostModal profiles={profiles} onClose={() => setModal(null)} onCreate={async (draft) => {
      ...
      const post: InstagramPost = { id: `ig-post-${Date.now()}`, authorId: author.id, media: draft.media, ratio: draft.ratio, caption: draft.caption, location: draft.location || undefined, createdAt: Date.now(), likes: 0, comments: [] };
      updateDatabase(current => ({ ...current, posts: [post, ...current.posts] })); setModal(null); setView('feed'); setNotice('Publication partagée.');
      const generated = await generateInstagramComments(post.caption, author, profiles);
      editPost(post.id, { comments: generated.map((comment, index) => ({ id: `ig-ai-${Date.now()}-${index}`, authorId: comment.authorId, text: comment.text, createdAt: Date.now(), likes: 0 })) });
    }} />}
    {modal === 'story' && <CreateStoryModal profiles={profiles} onClose={() => setModal(null)} onCreate={story => { updateDatabase(current => ({ ...current, stories: [story, ...current.stories] })); setModal(null); setNotice('Story ajoutée.'); }} />}
...
function ProfileView({ profile, profiles, posts, highlights, editor, onBack, onEdit, onPost, onFollow }: { profile: InstagramProfile; profiles: InstagramProfile[]; posts: InstagramPost[]; highlights: Array<{ id: string; title: string; cover: string }>; editor: boolean; onBack: () => void; onEdit: () => void; onPost: (post: InstagramPost) => void; onFollow: () => void }) {
  return <section className="ig-profile">
    ...
    <div className="ig-profile-bio"><b>{profile.displayName}</b><small>{profile.category}</small><p>{profile.bio}</p>{profile.link && <a href={profile.link} target="_blank" rel="noreferrer">{profile.link}</a>}</div>
...
function CreateStoryModal({ profiles, onClose, onCreate }: { profiles: InstagramProfile[]; onClose: () => void; onCreate: (story: InstagramStory) => void }) {
  const [authorId, setAuthorId] = useState(profiles[0]?.id ?? '');
  ...
  return <Overlay title="Créer une story" onClose={onClose}><form className="ig-form" onSubmit={event => { event.preventDefault(); if (authorId && media) onCreate({ id: `ig-story-${Date.now()}`, authorId, media, text: text.trim() || undefined, active: true, createdAt: Date.now() }); }}>
    ...
    <label>Média local<select value={media} onChange={event => setMedia(event.target.value)}>{availableMedia.map(item => <option key={item}>{item}</option>)}</select></label>
...
function EditProfileModal({ profile, profiles, onClose, onSave }: { profile: InstagramProfile; profiles: InstagramProfile[]; onClose: () => void; onSave: (profile: InstagramProfile) => void }) {
  ...
  return <Overlay title="Éditer le profil" onClose={onClose}><form className="ig-form" onSubmit={event => { event.preventDefault(); onSave(draft); }}>
    ...
    <label>Type de compte<select value={draft.accountType} onChange={event => update('accountType', event.target.value as InstagramProfile['accountType'])}>{['joueur', 'joueuse', 'club', 'coach', 'président', 'personnalité publique', 'femme/compagne', 'média'].map(item => <option key={item}>{item}</option>)}</select></label>
    ...
    <div className="ig-form-two"><label>Réputation<select value={draft.reputation} onChange={event => update('reputation', event.target.value as InstagramProfile['reputation'])}>{['populaire', 'arrogant', 'discret', 'controversé', 'leader', 'clash'].map(item => <option key={item}>{item}</option>)}</select></label><label>Personnalité<select value={draft.personality} onChange={event => update('personality', event.target.value as InstagramProfile['personality'])}>{['familier', 'corpo', 'provocateur', 'timide'].map(item => <option key={item}>{item}</option>)}</select></label></div>
    ...
    <label>Relations<select value="" onChange={event => { const [profileId, type] = event.target.value.split('|'); if (profileId && type) update('relations', [...draft.relations.filter(item => item.profileId !== profileId), { profileId, type: type as InstagramProfile['relations'][number]['type'] }]); }}>{<option value="">Ajouter une relation…</option>}{otherProfiles.flatMap(item => ['coéquipier', 'club lié', 'rival', 'couple', 'ami proche', 'coach', 'famille'].map(type => <option key={`${item.id}-${type}`} value={`${item.id}|${type}`}>{item.displayName} · {type}</option>))}</select></label>
