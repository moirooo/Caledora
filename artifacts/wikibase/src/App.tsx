...
function Shell({ children }: { children: ReactNode }) {
  ...
  const isHome = location === '/' || location === '/twitter' || location === '/airways';
...
function Dashboard() {
  ...
  const handleApp = (app: DashApp) => {
    ...
    if (app.id === 'airways')   navigate('/airways');
...
function FakeQR({ seed = 0, size = 100 }: { seed?: number; size?: number }) {
  const n = 25; const cell = size / n;
  const rects: { r: number; c: number }[] = [];
  const finderAt = (r0: number, c0: number) => {
    for (let r = 0; r < 7; r++) for (let c = 0; c < 7; c++)
      if (r===0||r===6||c===0||c===6||(r>=2&&r<=4&&c>=2&&c<=4)) rects.push({ r: r0+r, c: c0+c });
  };
  finderAt(0,0); finderAt(0,18); finderAt(18,0);
  for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
    if ((r<8&&c<8)||(r<8&&c>=17)||(r>=17&&c<8)) continue;
    if (r===6||c===6) { if ((r+c)%2===0) rects.push({r,c}); continue; }
    if (((r*23+c*17+seed+(r^c))%3)!==0) rects.push({r,c});
  }
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <rect width={size} height={size} fill="white" rx="3"/>
      {rects.map(({r,c},i) => <rect key={i} x={c*cell+0.3} y={r*cell+0.3} width={cell-0.6} height={cell-0.6} fill="#17365D"/>)}
    </svg>
  );
}

type AirwaysDest   = { code: string; city: string; country: string };
type AirwaysFlight = { num: string; dep: string; arr: string; duration: string; aircraft: string };
type AirwaysClass  = 'Economy' | 'Premium Economy' | 'Business' | 'Première';
type AirwaysSel    = { dest: AirwaysDest; flight: AirwaysFlight; cls: AirwaysClass; seat: string; gate: string; date: string };

const AW_DESTS: AirwaysDest[] = [
  { code:'CDG', city:'Paris',        country:'France'       },
  { code:'JFK', city:'New York',     country:'États-Unis'   },
  { code:'HND', city:'Tokyo',        country:'Japon'        },
...
const AW_FLIGHTS: Record<string, AirwaysFlight[]> = {
  CDG: [
    { num:'CW 312', dep:'07:10', arr:'10:25',    duration:'3h15',  aircraft:'Airbus A321neo'  },
    { num:'CW 318', dep:'13:40', arr:'16:55',    duration:'3h15',  aircraft:'Airbus A321neo'  },
    { num:'CW 324', dep:'20:05', arr:'23:20',    duration:'3h15',  aircraft:'Airbus A321neo'  },
  ],
  JFK: [
    { num:'CW 780', dep:'10:50', arr:'19:35',    duration:'8h45',  aircraft:'Airbus A380'     },
    { num:'CW 782', dep:'22:15', arr:'07:00+1',  duration:'8h45',  aircraft:'Airbus A350-900' },
  ],
  HND: [
    { num:'CW 540', dep:'11:25', arr:'09:55+1',  duration:'12h30', aircraft:'Airbus A380'     },
    { num:'CW 542', dep:'23:55', arr:'22:25+1',  duration:'12h30', aircraft:'Airbus A350-900' },
  ],
  FCO: [
    { num:'CW 210', dep:'06:30', arr:'09:15',    duration:'2h45',  aircraft:'Airbus A321neo'  },
    { num:'CW 214', dep:'14:00', arr:'16:45',    duration:'2h45',  aircraft:'Airbus A321neo'  },
  ],
  LHR: [
    { num:'CW 106', dep:'07:00', arr:'09:15',    duration:'2h15',  aircraft:'Airbus A321neo'  },
    { num:'CW 108', dep:'12:30', arr:'14:45',    duration:'2h15',  aircraft:'Airbus A321neo'  },
    { num:'CW 112', dep:'18:55', arr:'21:10',    duration:'2h15',  aircraft:'Airbus A321neo'  },
  ],
  MAD: [
    { num:'CW 260', dep:'09:15', arr:'11:45',    duration:'2h30',  aircraft:'Airbus A321neo'  },
    { num:'CW 264', dep:'17:30', arr:'20:00',    duration:'2h30',  aircraft:'Airbus A321neo'  },
  ],
  LAX: [
    { num:'CW 820', dep:'13:45', arr:'05:30+1',  duration:'11h45', aircraft:'Airbus A380'     },
    { num:'CW 822', dep:'23:00', arr:'14:45+1',  duration:'11h45', aircraft:'Airbus A350-900' },
  ],
};

const AW_SEATS: Record<AirwaysClass, string[]> = {
  'Economy':         ['24C','27B','31A','33E','35F','28D'],
  'Premium Economy': ['12A','12C','14D','15A','11F'],
  'Business':        ['3A','3C','5D','7A','6C'],
  'Première':        ['1A','1C','2A','2C'],
};

function AirwaysPage() {
  const [, navigate] = useLocation();
  const BASE = import.meta.env.BASE_URL.replace(/\/$/, '');
  const gold = '#C9A84C'; const navy = '#17365D';

  const [screen,   setScreen]   = useState<'search' | 'flights' | 'boarding'>('search');
  const [destCode, setDestCode] = useState('CDG');
  const initDate = () => { const d = new Date(); d.setDate(d.getDate()+3); return d.toISOString().slice(0,10); };
  const [date,     setDate]     = useState(initDate);
  const [cls,      setCls]      = useState<AirwaysClass>('Economy');
  const [sel,      setSel]      = useState<AirwaysSel | null>(null);
  const [pax,      setPax]      = useState('DUPONT Jean');
  const [editing,  setEditing]  = useState(false);

  const dest    = AW_DESTS.find(d => d.code === destCode)!;
  const flights = AW_FLIGHTS[destCode] ?? [];

  const pickFlight = (f: AirwaysFlight) => {
    const seat = AW_SEATS[cls][Math.floor(Math.random() * AW_SEATS[cls].length)];
    const gate = ['A12','B07','B23','C04','D18','E02'][Math.floor(Math.random()*6)];
    setSel({ dest, flight: f, cls, seat, gate, date });
    setScreen('boarding');
  };

  const fmtDate = (iso: string) => new Date(iso).toLocaleDateString('fr-FR', { day:'2-digit', month:'short', year:'numeric' }).toUpperCase();
  const qrSeed  = sel ? sel.flight.num.charCodeAt(3) + sel.seat.charCodeAt(0) : 42;

  const glass: React.CSSProperties = { background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.13)', backdropFilter:'blur(20px)' };
  const bg = `linear-gradient(160deg, ${navy} 0%, #0f2540 55%, #0a1a2e 100%)`;

  const AWHeader = () => (
    <div className="flex items-center justify-between px-6 py-4 shrink-0" style={{ background:navy, borderBottom:`2px solid ${gold}` }}>
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/')} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm font-medium">
          <ArrowLeft size={16}/><span className="hidden sm:inline">Hub</span>
        </button>
        <div className="w-px h-5 bg-white/20"/>
        <img src={`${BASE}/images/airways2.jpg`} alt="" className="h-8 w-8 rounded-lg object-cover"/>
        <div>
          <div className="text-white font-black text-sm tracking-widest">CALEDORA AIRWAYS</div>
          <div className="text-[10px] tracking-widest font-semibold" style={{color:gold}}>★ SKYTEAM MEMBER</div>
        </div>
      </div>
      <div className="text-right hidden sm:block">
        <div className="text-[10px] text-white/40 tracking-widest uppercase">Programme</div>
        <div className="font-bold text-sm" style={{color:gold}}>Aurora Miles</div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex flex-col" style={{background:bg}}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        <div className="w-full max-w-lg">
          <h1 className="text-white text-3xl font-black mb-0.5 tracking-tight">Réservez votre vol</h1>
          <p className="mb-8 text-sm font-semibold tracking-widest" style={{color:gold}}>CALEDORA CITY → LE MONDE</p>

          <div className="rounded-3xl p-6 space-y-4" style={glass}>
            {/* Origin */}
            <div>
              <div className="text-[10px] tracking-widest text-white/40 uppercase mb-1.5">Départ</div>
              <div className="flex items-center gap-3 rounded-2xl px-4 py-3" style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.10)'}}>
                <span className="text-lg">✈️</span>
                <div>
                  <div className="text-white font-bold text-sm">Caledora City — Hub T2</div>
                  <div className="text-white/35 text-[11px]">CAL · Terminal 2 · Hub international</div>
                </div>
              </div>
            </div>
            ...
            {/* Destination */}
            <div>
              <div className="text-[10px] tracking-widest text-white/40 uppercase mb-1.5">Destination</div>
              <select value={destCode} onChange={e => setDestCode(e.target.value)}
                className="w-full rounded-2xl px-4 py-3 text-white font-bold text-sm outline-none cursor-pointer"
                style={{background:'rgba(255,255,255,0.1)',border:'1px solid rgba(255,255,255,0.15)'}}>
                {AW_DESTS.map(d => <option key={d.code} value={d.code} style={{background:navy}}>{d.city} — {d.code} · {d.country}</option>)}
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="text-[10px] tracking-widest text-white/40 uppercase mb-1.5">Date de départ</div>
                <input type="date" value={date} onChange={e => setDate(e.target.value)}
                  className="w-full rounded-2xl px-4 py-3 text-white font-bold text-sm outline-none"
                  style={{background:'rgba(255,255,255,0.1)',border:'1px solid rgba(255,255,255,0.15)',colorScheme:'dark'}}/>
              </div>
              <div>
                <div className="text-[10px] tracking-widest text-white/40 uppercase mb-1.5">Classe</div>
                <select value={cls} onChange={e => setCls(e.target.value as AirwaysClass)}
                  className="w-full rounded-2xl px-4 py-3 text-white font-bold text-sm outline-none cursor-pointer"
                  style={{background:'rgba(255,255,255,0.1)',border:'1px solid rgba(255,255,255,0.15)'}}>
                  {(['Economy','Premium Economy','Business','Première'] as AirwaysClass[]).map(c =>
                    <option key={c} value={c} style={{background:navy}}>{c}</option>)}
                </select>
              </div>
            </div>

            <button onClick={() => setScreen('flights')}
              className="w-full py-4 rounded-2xl text-sm font-black tracking-widest transition-all hover:opacity-90 active:scale-[0.98]"
              style={{background:`linear-gradient(135deg,${gold},#E8C56A)`,color:navy}}>
              VOIR LES VOLS DISPONIBLES →
            </button>
          </div>

          <div className="grid grid-cols-3 gap-3 mt-5">
            {[['✈️','Vols directs','7 destinations'],['🌍','SkyTeam','Alliance mondiale'],['⭐','Aurora Miles','Programme fidélité']].map(([e,t,s]) => (
              <div key={t} className="rounded-2xl p-3 text-center" style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)'}}>
                <div className="text-2xl mb-1">{e}</div>
                <div className="text-white text-xs font-bold">{t}</div>
                <div className="text-white/35 text-[10px] mt-0.5">{s}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  /* ── Flights ── */
  if (screen === 'flights') return (
    <div className="min-h-screen flex flex-col" style={{background:bg}}>
      <AWHeader/>
      <div className="flex-1 px-4 py-8 w-full max-w-lg mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => setScreen('search')} className="text-white/50 hover:text-white transition-colors"><ArrowLeft size={20}/></button>
          <div>
            <h2 className="text-white text-xl font-black">CAL → {destCode}</h2>
            <p className="text-white/40 text-xs tracking-wide">{fmtDate(date)} · {cls} · {flights.length} vol{flights.length>1?'s':''} direct{flights.length>1?'s':''}</p>
          </div>
        </div>

        <div className="space-y-4">
          {flights.map((f, i) => (
            <div key={f.num} className="rounded-3xl overflow-hidden" style={glass}>
              <div className="px-5 pt-4 pb-3 flex items-center justify-between" style={{borderBottom:'1px solid rgba(255,255,255,0.08)'}}>
                <div className="flex items-center gap-2">
                  <span className="text-white font-black text-lg">{f.num}</span>
                  {i===0 && <span className="text-[10px] font-black px-2 py-0.5 rounded-full" style={{background:gold,color:navy}}>MEILLEUR PRIX</span>}
                </div>
                <span className="text-white/35 text-xs">{f.aircraft}</span>
              </div>
              <div className="px-5 py-4 flex items-center gap-3">
                <div className="text-center w-16">
                  <div className="text-white text-2xl font-black">{f.dep}</div>
                  <div className="text-white/40 text-xs mt-0.5">CAL</div>
                </div>
                <div className="flex-1 flex flex-col items-center gap-1">
                  <div className="text-white/30 text-[11px]">{f.duration} · Direct</div>
                ...
                <div className="text-center w-16">
                  <div className="text-white text-2xl font-black">{f.arr}</div>
                  <div className="text-white/40 text-xs mt-0.5">{destCode}</div>
                </div>
              </div>
              <div className="px-5 pb-4 flex items-center justify-between">
                <div className="flex gap-3 text-[11px] text-white/35">
                  <span>🧳 Bagage cabine</span><span>⭐ Aurora</span>
                </div>
                <button onClick={() => pickFlight(f)}
                  className="px-5 py-2 rounded-full text-sm font-black transition-all hover:opacity-90"
                  style={{background:`linear-gradient(135deg,${gold},#E8C56A)`,color:navy}}>
                  Sélectionner
                </button>
              </div>
        ...
        <div className="mt-5 rounded-2xl px-4 py-3 text-[11px] text-white/35" style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)'}}>
          🧳 Franchise bagages {cls==='Economy'?'soute 23 kg en option':cls==='Premium Economy'?'1 × 23 kg inclus':cls==='Business'?'2 × 32 kg inclus':'3 × 32 kg inclus'} · Programme Aurora Miles applicable
        </div>
      </div>
    </div>
  );
  ...
  /* ── Boarding pass ── */
  const detailRows = [
    ['VOL',sel!.flight.num],['DATE',fmtDate(sel!.date)],['CLASSE',sel!.cls==='Première'?'1ʳᵉ':sel!.cls],
    ['DÉPART',sel!.flight.dep],['ARRIVÉE',sel!.flight.arr],['DURÉE',sel!.flight.duration],
    ['SIÈGE',sel!.seat],['PORTE',sel!.gate],['TERMINAL','T2'],
  ];
  return (
    <div className="min-h-screen flex flex-col" style={{background:bg}}>
      <AWHeader/>
      <div className="flex-1 flex flex-col items-center py-8 px-4">
        <div className="w-full max-w-sm">
          <div className="flex items-center gap-3 mb-5">
            <button onClick={() => setScreen('flights')} className="text-white/50 hover:text-white transition-colors"><ArrowLeft size={20}/></button>
            <span className="text-white/50 text-sm tracking-wide">Carte d'embarquement</span>
          </div>

          {/* CARD */}
          <div className="rounded-3xl overflow-hidden shadow-2xl" style={{boxShadow:`0 30px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.1)`}}>
            {/* Top navy strip */}
            <div className="px-6 py-4 flex items-center justify-between" style={{background:navy}}>
              <div>
                <div className="text-white font-black text-sm tracking-widest">CALEDORA AIRWAYS</div>
                <div className="text-[10px] tracking-widest font-bold" style={{color:gold}}>✦ SKYTEAM MEMBER</div>
              </div>
              <img src={`${BASE}/images/airways2.jpg`} alt="" className="h-10 w-10 rounded-xl object-cover"/>
            </div>

            {/* Route hero */}
            <div className="px-6 py-5" style={{background:`linear-gradient(135deg,${navy} 0%,#1e4d82 100%)`}}>
              <div className="text-center mb-4">
                {editing
                  ? <input autoFocus value={pax} onChange={e => setPax(e.target.value.toUpperCase())}
                      onBlur={() => setEditing(false)} onKeyDown={e => e.key==='Enter'&&setEditing(false)}
                      className="w-full text-center text-white font-black text-lg rounded-xl px-3 py-1.5 outline-none"
                      style={{background:'rgba(255,255,255,0.12)',border:`1px solid ${gold}60`}}/>
                  : <button onClick={() => setEditing(true)} className="text-white font-black text-xl tracking-wider hover:opacity-75 transition-opacity">
                      {pax} <span className="text-white/30 text-sm font-normal">✎</span>
                    </button>
                }
                <div className="text-white/35 text-[10px] tracking-widest mt-0.5">PASSAGER · PASSENGER</div>
              </div>
              <div className="flex items-center justify-between gap-2">
                <div className="text-center">
                  <div className="text-white font-black leading-none" style={{fontSize:'3.2rem',letterSpacing:'-2px'}}>CAL</div>
                  <div className="text-white/40 text-[11px] mt-1 tracking-wide">Caledora City</div>
                </div>
                <div className="flex-1 flex flex-col items-center">
                  <span style={{color:gold,fontSize:'1.5rem'}}>✈</span>
                  <div className="h-px w-full mt-1" style={{background:`linear-gradient(90deg,transparent,${gold},transparent)`}}/>
                </div>
                <div className="text-center">
                  <div className="text-white font-black leading-none" style={{fontSize:'3.2rem',letterSpacing:'-2px'}}>{sel!.dest.code}</div>
                  <div className="text-white/40 text-[11px] mt-1 tracking-wide">{sel!.dest.city}</div>
                </div>
              </div>
            </div>

            {/* Detail grid */}
            <div className="px-5 py-5 grid grid-cols-3 gap-y-4" style={{background:'#f7f6f2'}}>
              {detailRows.map(([label,value]) => (
                <div key={label} className="text-center">
                  <div className="text-[9px] font-black tracking-widest mb-0.5" style={{color:navy+'70'}}>{label}</div>
                  <div className="font-black text-sm" style={{color:navy}}>{value}</div>
                </div>
              ))}
            </div>

            {/* Tear line */}
            <div className="flex items-center px-0" style={{background:'white'}}>
              <div className="w-6 h-6 rounded-full shrink-0 -ml-3" style={{background:bg}}/>
              <div className="flex-1 border-t-2 border-dashed mx-0.5" style={{borderColor:'#d4d0c7'}}/>
              <div className="w-6 h-6 rounded-full shrink-0 -mr-3" style={{background:bg}}/>
            </div>

            {/* QR section */}
            <div className="px-5 py-5 flex items-center gap-4" style={{background:'#f7f6f2'}}>
              <div className="rounded-xl overflow-hidden p-1" style={{background:'white',boxShadow:'0 2px 12px rgba(0,0,0,0.08)'}}>
                <FakeQR seed={qrSeed} size={96}/>
              </div>
              <div className="flex-1 text-right space-y-1">
                <div className="text-[9px] font-black tracking-widest" style={{color:navy+'60'}}>PROGRAMME AURORA</div>
                <div className="font-black text-sm" style={{color:navy}}>★ Aurora Miles</div>
                <div className="text-[9px] font-bold mt-3" style={{color:navy+'70'}}>EMBARQUEMENT</div>
                <div className="font-black text-3xl leading-none" style={{color:navy}}>45<span className="text-base font-bold"> min</span></div>
                <div className="text-[10px]" style={{color:navy+'50'}}>avant départ</div>
              </div>
            </div>

            {/* Aircraft footer */}
            <div className="px-6 pb-4 text-center" style={{background:'#f7f6f2'}}>
              <div className="text-[10px] tracking-widest font-bold" style={{color:navy+'45'}}>APPAREIL · {sel!.flight.aircraft.toUpperCase()}</div>
            </div>
          </div>
          ...
          <div className="flex gap-3 mt-5">
            <button onClick={() => window.print()}
              className="flex-1 py-3.5 rounded-2xl text-sm font-black tracking-wide flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
              style={{background:`linear-gradient(135deg,${gold},#E8C56A)`,color:navy}}>
              🖨️ Imprimer
            </button>
            <button onClick={() => { setSel(null); setScreen('search'); }}
              className="flex-1 py-3.5 rounded-2xl text-sm font-bold hover:bg-white/10 transition-colors"
              style={{background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.14)',color:'white'}}>
              Nouveau vol
            </button>
          </div>
          <p className="text-center text-white/20 text-[10px] mt-3 tracking-wide">SIMULATION · USAGE FICTIF UNIQUEMENT</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <LightboxProvider>
      <Shell>
        ...
        <Route path="/airways" component={AirwaysPage} />
